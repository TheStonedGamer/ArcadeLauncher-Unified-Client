...
extracts this version's section verbatim and posts it to Discord â€” so this file
...
- The call bar and the phone's call screen now say _why_ a call ended â€”
  unreachable, declined, or no answer â€” rather than silently disappearing.
...
- GIF picker in chat, powered by Tenor. Add your own Tenor key under
  **Settings â†’ Integrations** to enable it.
...
- Video calls can be placed directly: a **Video** button next to Call in a
  ...
  still works â€” it just rings as a voice call.
...
- **Settings â†’ Voice & Video** controls the microphone processing used on calls:
...
- Cloud saves now find your saves on their own: the launcher detects the usual
  ...
  **Set cloud save folderâ€¦**, **Set cloud save fileâ€¦** and **Use detected save
  location** so you can point it anywhere â€” including a single memory-card file.
...
- Friends is a window now, not a tab. The **Friends & Chat** button in the
  ...
  a friend opens that conversation in its own window â€” both float over the
...
- Settings tab in the header â€” use the profile menu in the top right.
...
- Xbox games now boot the disc image instead of starting the emulator with
  nothing loaded â€” the launch arguments the server sends were being dropped.
...
- Installed games that are not in the signed-in account's library now remain
  visible and carry an `Installed Â· not in library` marker in Library, Store,
  and game details.
...
- **The phone now shows who's signed in.** The companion's DMs tab was coming up
  empty even when friends were online, because it never loaded your friend list â€”
...
- **The phone's "Friends" tab is now "DMs"** â€” same friends and chat, clearer name.
...
- **Phone app sign-in.** The Android companion now signs in the same secure way
  the desktop launcher does â€” your password is proven to the server without ever
  leaving your phone â€” instead of failing at the login screen.
...
- **Install to your PC from your phone.** Open a game on the phone, pick which
  ...
  reports back â€” started, already installed, already downloading â€” so you know
...
- **The phone app is now downloadable straight from the release page.** Every
  ...
  alongside the Windows and Linux installers â€” no more digging through build
...
- **Video calls and screen sharing.** While you're in a voice call with a friend,
  the call bar now has **ðŸ“· Camera** and **ðŸ–¥ Share screen** buttons. Whatever's
...
- **Your week â€” a weekly recap of what you played.** The launcher now remembers
  ...
  filling in from your next game â€” earlier playtime totals are untouched.
...
- **Downloads now show game names instead of internal IDs.** The queue could fall
  back to showing raw ids like `pc-fdc100f88077` â€” most often on a fresh install,
...
- The last leftovers of built-in game streaming are gone from both the launcher
  ...
  in day-to-day use â€” Settings â†’ Remote Play still points you at **Moonlight**
...
- **Built-in game streaming has been removed.** The bundled stream engine, the
  ...
  â€” so the installer is smaller and updates are quicker. If you want to stream your
  games, Settings â†’ Remote Play now links you to **Moonlight** (the client) and
...
- **Multiple library folders, Steam-style.** Settings â†’ Storage lets you add an
  ...
  drive to put it on.
- **Move installed games between drives.** Right-click a game â†’ "Move install
  folderâ€¦", choose a target library, and watch a live progress bar as the files
...
- **Streaming to a PC on another network now actually completes the one-time
  ...
  rejects â€” so turning on "Let this PC be streamed" brings the host onto the
...
- **Host engine controls in Settings â†’ Stream from this PC.** A new "Host engine"
  ...
  automatically the first time you enable hosting â€” this is the manual fix-it
...
- **Hosting no longer silently uses a leftover Sunshine.** The host now always
  ...
  already running on the PC â€” which previously left "Stream from this PC" unable
...
- **Streaming host setup is now bundled with updates instead of downloaded on
  ...
  time â€” no separate download that could lag behind or fail when you enable it.
...
- **Stream a "My PCs" game with no PIN.** PCs signed into the same account now
  pair automatically the first time you hit Play â€” no more typing a PIN into
...
- When a remote PC needs pairing, Play now shows an inline **PIN prompt** and
  retries automatically once you enter it â€” instead of a window that flashed
  open and closed with a misleading "Streaming âœ“" message.
...
- **The "Let this PC be streamed" switch now stays on after you restart
  ...
  automatically the next time the app launches â€” so it's ready to stream to
...
- **A PC you stream to no longer shows "offline" while it's sitting ready.**
  ...
  ArcadeLauncher is running â€” not only while that PC has the _My PCs_ tab open â€”
...
- **Play your PCs from anywhere.** When a PC in _My PCs_ has no address on your
  ...
  encrypted mesh â€” no port forwarding, no VPN to install, nothing to type. The
...
- **Stream your Steam and Epic games from this PC.** Auto-detected Steam and Epic
  ...
  **Settings â†’ Stream from this PC â†’ Publish my library**, so you can launch and
  play them from your other devices.

### Changed

- **Hosting uses the Sunshine you already have.** When you turn on "Let this PC be
  streamed", the launcher now uses a Sunshine that's already installed â€” or already
  running â€” instead of downloading its own copy. If you started Sunshine yourself,
...
- **"Let this PC be streamed" now stays on.** The toggle in Settings used to flip
  straight back off â€” turning it on never actually kept this PC hosting. It now
...
- **Your PCs now find each other automatically.** Every PC signed into your
  ArcadeLauncher account shows up under **My PCs** on your other devices â€” no IP
...
- **Browse a PC's games even when it's asleep.** Each PC publishes the games
  ...
  per game.
- Turning on **Settings â†’ Stream from this PC** now publishes that machine's
...
- **Stream your PCs without a separate Moonlight install.** "Stream from host" now
  plays straight through the streaming engine bundled in the app â€” you get a live
...
- **The bundled streaming engine now actually streams.** Updated to its first
  streaming-capable build â€” previously it only handled host pairing.
...
- **A "My PCs" tab.** Your other PCs that you've paired for streaming now live in
  one place â€” each one expands to the games it has, with a Play button that streams
  it to this machine. Your personal remote-play list, Steam-style.
- **Host this PC for streaming.** A new **Settings â†’ Stream from this PC** section
  lets this machine be streamed to your other devices and publish your installed
  library as playable games â€” powered by a streaming engine bundled right in the
...
- **Pairing a streaming host is simpler and safer.** Pair with just the host's
  4-digit PIN â€” no Sunshine username or password needed anymore. Pairing now runs
...
- **A Chats tab in Social.** It's the first tab and opens by default, listing your
  active conversations newest-first with the latest message and unread counts â€” so
...
- **The updater now updates itself.** Because the Steam-style bootstrapper is what
  ...
  update â€” and Windows won't let an installer overwrite a running program, so the
...
- **Group voice calls.** Start a voice call from inside a room and everyone joins
  a single call â€” see who's connected, mute yourself, and leave from the in-call
  bar.
- **Save version history.** Each game's cloud-saves panel can now show a list of
  ...
  and restore an older one â€” your current save is snapshotted first, so a restore
  is itself undoable.
...
- **Cloud-save auto-sync.** Saves now sync automatically around play: the latest
  ...
  set how many versions to keep under **Settings â†’ Cloud saves**. Applies to
...
- **Docs:** refreshed the platform architecture reference (`ARCHITECTURE.md` and
  ...
  the server â€” it now runs in-process under `/requests` on the main server instead
...
- **Friends activity feed.** The social panel now has an **Activity** tab showing
  what your friends have been up to â€” recent sessions and game activity â€” so you
...
- **Game request status is now managed by admins on the server.** The Requests
  board no longer shows an inline status dropdown for administrators â€” request
...
- **Forgot your password?** The sign-in screen now has a **Forgot password?**
  ...
  (valid for 1 hour) to choose a new password â€” no admin needed. For your
...
- The first-run **welcome tour** now appears on your **first sign-in** instead of
  the first time the app opens, and is tracked **per account** â€” so you see it
...
- **Game invites, everywhere** (T12d follow-up): invite toasts now appear on any
  tab â€” not just Friends â€” because the social connection is held once at the app
...
- Groundwork for **group chats / channels**: the launcher can now understand
  multi-party rooms over the social gateway â€” being added to a room, members
  joining or leaving, renames, and a room being deleted â€” and keeps its room list
...
- Groundwork for **save version history**: the launcher can now keep a series of
  ...
  restore is itself undoable â€” the current save is snapshotted before it's
...
- Groundwork for **game invites**: the client can now understand a friend's
  ...
  visible UI yet â€” the invite toast and one-click **Join** land next.
...
- **Remote game streaming (Sunshine/Moonlight)** is now usable end to end. A new
  **Settings â†’ Streaming** section lets you pair with a Sunshine host PC by its
  4-digit PIN, see your paired hosts (and forget them), tell at a glance whether
  Moonlight is installed, and set your stream-quality defaults â€” resolution,
  frame rate, bitrate, window mode, and HDR. Once a host is paired, a **â–¶ Stream
...
- Groundwork for **remote game streaming** (Sunshine/Moonlight): the launcher
  ...
  whether a given game is streamable from it. No visible UI yet â€” this is the
...
- More streaming groundwork: the client can now construct the requests needed to
  ...
  certificate so the connection stays secure. Still no visible UI â€” wiring
  continues.
- The client can now actually talk to a Sunshine streaming host: pair with a PIN,
  list the games it offers, and add a game to it â€” over a secure connection that
...
- Streaming can now launch the **Moonlight** client: the launcher detects whether
  ...
  behind the scenes â€” the "Stream from host" button arrives next.
...
- In-client Game Requests board (pure core): browse/upvote/request games, a
  client-side platform filter, platform-scoped search, and community 1â€“5 star
  game ratings (averaged). UI to follow.
- Game Requests board transport: the launcher now talks to the Requests service
  using your existing sign-in (bearer token) â€” no separate board login â€” to list
...
- **Requests tab**: browse and vote on game requests without leaving the
  ...
  games 1â€“5 stars, filter by status or platform, and â€” if you're an admin â€” set
