...
// A deliberate mirror of the desktop client's src/features/social/embeds.ts:
// the same rules, so the same message embeds the same way on both. Two
// implementations of one contract is already one too many, so where they must
// both exist they should at least give the same answers.
//
// Three kinds: an image (GIF/PNG/JPEG/WebP/AVIF), a video file (MP4/WebM), and
// a YouTube video, which is a page rather than a file and so opens in the
// YouTube app rather than playing inline.
//
...
//
// Video files play inline; YouTube hands off to the YouTube app or browser,
// which is where a phone wants to watch one anyway.
