...
describe("callReducer — guards", () => {
  it("remembers a call was placed as a video call, at both ends", () => {
    expect(
      callReducer(IDLE_CALL, { type: "invite", peerId: 3, video: true })
        .wantsVideo,
    ).toBe(true);
    expect(
      callReducer(IDLE_CALL, { type: "incoming", peerId: 3, video: true })
        .wantsVideo,
    ).toBe(true);
  });

  it("ignores invite while already busy", () => {
    const busy = callReducer(IDLE_CALL, {
      type: "invite",
      peerId: 7,
      video: false,
    });
    expect(callReducer(busy, { type: "invite", peerId: 9, video: false })).toBe(
      busy,
    );
    expect(
      callReducer(busy, { type: "incoming", peerId: 9, video: false }),
    ).toBe(busy);
  });
  it("accept only valid from ringing", () => {
    expect(callReducer(IDLE_CALL, { type: "accept" })).toBe(IDLE_CALL);
    const inviting = callReducer(IDLE_CALL, {
      type: "invite",
      peerId: 7,
      video: false,
    });
...
describe("callReducer — video (T12e)", () => {
  ...
  it("allows video while still connecting (renegotiation can start early)", () => {
    const connecting = callReducer(
      callReducer(IDLE_CALL, { type: "invite", peerId: 7, video: false }),
      {
        type: "remoteAccept",
      },
    );
    expect(
      callReducer(connecting, { type: "localVideo", mode: "camera" })
        .localVideo,
    ).toBe("camera");
  });
  ...
  it("ignores video events with no peer connection behind them", () => {
    for (const state of [
      IDLE_CALL,
      callReducer(IDLE_CALL, { type: "invite", peerId: 7, video: false }),
      callReducer(IDLE_CALL, { type: "incoming", peerId: 3, video: false }),
      callReducer(connected(), { type: "hangup" }),
    ]) {
      expect(callReducer(state, { type: "localVideo", mode: "camera" })).toBe(
        state,
      );
      expect(callReducer(state, { type: "remoteVideo", mode: "screen" })).toBe(
        state,
      );
    }
  });
  ...
  it("does not leak video state into the next call", () => {
    let s = callReducer(connected(), { type: "localVideo", mode: "screen" });
    s = callReducer(s, { type: "hangup" });
    s = callReducer(s, { type: "incoming", peerId: 9, video: false });
...
describe("parseSignal", () => {
  it("parses simple kinds", () => {
    ...
    expect(parseSignal({ kind: "invite", video: true })).toEqual({
      kind: "invite",
      video: true,
    });
  ...
  it("parses video announcements", () => {
    ...
    expect(parseSignal({ kind: "video", mode: "camera" })).toEqual({
      kind: "video",
      mode: "camera",
    });
    expect(parseSignal({ kind: "video", mode: "none" })).toEqual({
      kind: "video",
      mode: "none",
    });
  });
